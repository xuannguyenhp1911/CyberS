...
import { createConfigScannerV3 } from '../../../../../../../../services/Configs/Binance/V3/scannerService';
...
function CreateStrategy({
    ...
    const DEFAULT_REMEMBER_VALUE = JSON.parse(localStorage.getItem("Binance_V3_Scan"))
    ...
    const handleSubmitCreate = async (data, addMore = false) => {
        ...
        if ((onlyPairsSelected.length > 0 || groupCoinOnlyPairsID) && botList.length > 0) {
            ...
            try {
                ...
                if (data.Remember) {
                    localStorage.setItem("Binance_V3_Scan", JSON.stringify(data))
                }
            }
            catch (err) {
                dispatch(addMessageToast({
