...
import { updateStrategiesMultiple, deleteStrategiesMultiple, copyMultipleStrategiesToSymbol, copyMultipleStrategiesToBot } from '../../../../../../../../services/Configs/Binance/V3/configOldService';
...
import styles from "./EditMulTreeItem.module.scss"
function EditMulTreeItem({
    ...
    const handleGetAllBot = async () => {
        try {
            const res = await getAllBotActive("Binance_V3")
    ...
    const handleUpdate = async () => {
        ...
        const newDataUpdateAllObject = {}
        try {
            const newData = handleDataCheckTreeSelected.map((dataCheckTreeItem) => {
                const id = dataCheckTreeItem._id
                const parentID = dataCheckTreeItem.parentID
                const UpdatedFields = filterDataRowList.map(filterRow => {
                    ...
                    if (typeof (valueHandle) === "number" && !["StopLose", ].includes(filedValue)) {
                ...
                newDataUpdateAllObject[`${parentID}-${id}`] = UpdatedFields
                return {
                    id,
                    parentID,
                    UpdatedFields
            ...
            if (checkValueMin) {
                ...
                if (status === 200) {
                    const newDataUpdateAll = dataCheckTreeDefaultRef.current.map(itemParent => {
                        return {
                            ...itemParent,
                            children: itemParent.children?.map(item => {
                                const newData = newDataUpdateAllObject[`${itemParent._id}-${item._id}`]
                                if (newData) {
                                    return newData
                                }
                                return item
                    ...
                    dataCheckTreeDefaultRef.current = newDataUpdateAll
                    handleFilterAll()
                    closeDialog(false)
                }
            }
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            const newData = handleDataCheckTreeSelected.map((dataCheckTreeItem) => {
                const id = dataCheckTreeItem._id
                const parentID = dataCheckTreeItem.parentID
                newDataUpdateAllObject[`${parentID}-${id}`] = true
                return {
                    id,
                    parentID,
                }
            ...
            if (status === 200) {
                const newDataUpdateAll = dataCheckTreeDefaultRef.current.map(itemParent => {
                    return {
                        ...itemParent,
                        children: itemParent.children?.filter(item => !newDataUpdateAllObject[`${itemParent._id}-${item._id}`])
                    }
            ...
            <div style={{
                ...
                >
                    <div className={styles.radioGroupActionItem}>
                        <FormControlLabel className={styles.radioItem} value="Update" control={<Radio />} label="Update" />
                        <FormControlLabel className={styles.radioItem} value="Delete" control={<Radio />} label="Delete" />
                        <FormControlLabel className={styles.radioItem} value="Copy" control={<Radio />} label="Copy To" />
                    </div>
                    <div className={styles.radioGroupActionItem}>
                        <FormControlLabel className={styles.radioItem} value="ON" control={<Radio />} label="ON" />
                        <FormControlLabel className={styles.radioItem} value="OFF" control={<Radio />} label="OFF" />
                        <FormControlLabel className={styles.radioItem} value="Beta" control={<Radio />} label="Beta" />
                    </div>
                </RadioGroup>
            </div>
...
export default memo(EditMulTreeItem); 