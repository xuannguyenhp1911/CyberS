...
function ConfigThongKeBinanceOld() {
    ...
    const handleGetAllBotByUserID = () => {
        ...
        getAllBotOnlyApiKey("Binance_V3")
            .then(res => {
    ...
    const handleFilterAll = () => {
        ...
        const listData = dataCheckTreeDefaultRef.current.reduce((acc, data) => {
            ...
            const filteredChildren = data?.children?.filter(item => {
                ...
                // const checkSearch = searchDebounce === "" || searchDebounce.includes(`${data._id}-${item._id}`);
...
export default ConfigThongKeBinanceOld;