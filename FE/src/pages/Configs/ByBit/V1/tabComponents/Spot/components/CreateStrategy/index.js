...
import { createStrategiesSpot, getSpotBorrowCheck } from "../../../../../../../../services/Configs/ByBIt/V1/spotService";
...
function CreateStrategy({
    ...
    const DEFAULT_REMEMBER_VALUE = JSON.parse(localStorage.getItem("ByBit_V1_Spot"))
    ...
    const handleGetSymbolList = async (hasMargin = false) => {
        try {
            ...
            const newData = newSymbolList.filter(item => hasMargin ? true : item.market == "Spot")
    ...
    const handleSubmitCreate = async data => {
        ...
        if (symbolGroupData.length > 0 && botList.length > 0) {
            try {
                ...
                if (data.Remember) {
                    localStorage.setItem("ByBit_V1_Spot", JSON.stringify(data))
                }
            }
            catch (err) {
                dispatch(addMessageToast({
