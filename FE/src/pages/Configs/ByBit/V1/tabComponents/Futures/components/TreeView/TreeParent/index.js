...
import { updateStrategiesMultipleSpot,addToBookmarkSpot,removeToBookmarkSpot, getLeverOKXFutures } from '../../../../../../../../../services/Configs/ByBIt/V1/futuresService';
...
function TreeParent({
    ...
    const handleGetLever = async (instId) => {
        try {
            ...
            treeData.children.forEach(data=>{
                ...
                botListData[botID] = botData
            })
            await Promise.allSettled(["cross", "isolated"].map(mgnMode => {
                botListData && Object.values(botListData)?.map(async botData => {
                    try {
                        const res = await getLeverOKXFutures({
                            instId,
                            mgnMode,
                            botData
                        })
                        const { data } = res.data
                        const leverID = `${botData._id}-${instId}`
                        setLeverByBotAndSymbol(prevState => {
                            const newLeverByBotAndSymbol = { ...prevState };
                            newLeverByBotAndSymbol[leverID] = newLeverByBotAndSymbol[leverID] || {};
                            newLeverByBotAndSymbol[leverID][mgnMode] = data;
                            return newLeverByBotAndSymbol;
                        });
                    } catch (error) {
                        dispatch(addMessageToast({
                            status: 500,
                            message: "Get Lever Error",
                        }))
                    }
                })
            }))
            ...
            >
                ...
                <div
                    ...
                    onClick={e => {
                        ...
                        // if (!leverByBotAndSymbol?.[symbol]?.cross) {
                        //     handleGetLever(symbol)
                        // }
                    }}
                >
                        ...
                        <Table className={styles.tableData}>
                            <TableHead >
                                <TableRow>
                                    ...
                                    <TableCell className={styles.tableHeadCell}>Amount Auto</TableCell>
                                    <TableCell className={styles.tableHeadCell}>Amount Exp (m)</TableCell>
                                    <TableCell className={styles.tableHeadCell}>Limit ($)</TableCell>
                                    <TableCell className={styles.tableHeadCell}>Expire (m)</TableCell>
                                    ...
                                    <TableCell className={styles.tableHeadCell}>OC Pump</TableCell>
