...
import { createStrategies } from "../../../../../../../../services/Configs/ByBIt/V3/configService";
...
function CreateStrategy({
    ...
    const DEFAULT_REMEMBER_VALUE = JSON.parse(localStorage.getItem("ByBit_V3_Config"))
    ...
    const handleSubmitCreate = async data => {
        ...
        if (symbolGroupData.length > 0 && botList.length > 0) {
            try {
                ...
                if (data.Remember) {
                    localStorage.setItem("ByBit_V3_Config", JSON.stringify(data))
                }
            }
            catch (err) {
                dispatch(addMessageToast({
