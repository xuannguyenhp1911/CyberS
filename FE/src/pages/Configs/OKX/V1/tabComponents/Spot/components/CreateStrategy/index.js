...
import { FormControl, FormLabel, Autocomplete, TextField, Button, Checkbox, RadioGroup, FormControlLabel, Radio, MenuItem, Switch, InputAdornment, Table, TableBody, TableCell, TableHead, TableRow, CircularProgress } from "@mui/material"
...
import { createStrategiesSpot, getSpotBorrowCheck } from "../../../../../../../../services/Configs/OKX/V1/spotService";
...
function CreateStrategy({
    ...
    const DEFAULT_REMEMBER_VALUE = JSON.parse(localStorage.getItem("OKX_V1_Spot"))
    ...
    const spotListObjectRef = useRef({})
    ...
    const handleGetSymbolList = async (hasMargin = false) => {
        try {
            ...
            const newSymbolList = symbolListDataRes.reduce((pre, cur) => {
                ...
                spotListObjectRef.current[symbol] = spotListObjectRef.current[symbol] ? 2 : 1
                return pre
            }, [])

            const newData = newSymbolList.filter(item => hasMargin ? true : spotListObjectRef.current[item.name] == 1)

            symbolListRef.current = newData
            setSymbolGroupDataList({
                label: "Symbol",
                list: newData
            })
    ...
    const handleSubmitCreate = async data => {
        ...
        if (symbolGroupData.length > 0 && botList.length > 0) {
            try {
                ...
                if (data.Remember) {
                    localStorage.setItem("OKX_V1_Spot", JSON.stringify(data))
                }
            }
            catch (err) {
                dispatch(addMessageToast({
                            ...
                            <FormLabel className={styles.label}>Only pairs</FormLabel>
                            {
                                <div style={{ marginLeft: "6px" }}>
                                    ...
                                    <Switch
                                        size='small'
                                        onChange={(e) => {
                                            handleGetSymbolList(e.target.checked)
                                        }}
                                    />
                                </div>
                            }
                ...
                <div className={styles.formMainData}>
                    ...
                    <div className={clsx(styles.formControl, styles.formMainDataItem, styles.formMainDataSmall)} >
                        ...
                        <FormControl className={clsx(styles.formMainDataSmallItem)}>
                            ...
                            />
                            {/* {errors.AmountExpire?.type === 'required' && <p className="formControlErrorLabel">The Amount expire Required.</p>} */}
