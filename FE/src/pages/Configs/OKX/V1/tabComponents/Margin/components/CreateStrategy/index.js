...
import { FormControl, FormLabel, Autocomplete, TextField, Button, Checkbox, RadioGroup, FormControlLabel, Radio, MenuItem, Switch, InputAdornment, Table, TableBody, TableCell, TableHead, TableRow, CircularProgress, Chip } from "@mui/material"
...
import { createStrategiesSpot, getMarginBorrowCheck } from "../../../../../../../../services/Configs/OKX/V1/marginService";
...
function CreateStrategy({
    ...
    const DEFAULT_REMEMBER_VALUE = JSON.parse(localStorage.getItem("OKX_V1_Margin"))
    ...
    const [PositionSide, setPositionSide] = useState(DEFAULT_REMEMBER_VALUE?.PositionSide || positionSideList[0].value); 
    ...
    const handleSubmitCreate = async data => {
        ...
        if (symbolGroupData.length > 0 && botList.length > 0) {
            try {
                ...
                if (data.Remember) {
                    localStorage.setItem("OKX_V1_Margin", JSON.stringify(newData))
                }
            }
            catch (err) {
                dispatch(addMessageToast({
        ...
        try {
            ...
            if (status === 200) {
                setSpotMaxTradeAmountList(newData.map(item => ({
                    ...item,
                    spotMaxTradeAmount: Math.abs((+item.spotMaxTradeAmount).toFixed(3)),
                    availBal: Math.abs((+item.availBal).toFixed(3)),
                })))
            }
        }
        catch (err) {
    ...
    }, [symbolGroupData, botList, PositionSide]);
                ...
                <div className={styles.formMainData}>
                      ...
                      <div style={{
                        ...
                        }}>
                            {
                                positionSideList.map(side => {
                                    ...
                                    return (
                                        <Chip
                                            variant={PositionSide == value ? "filled" : "outlined"}
                                            label={side.name}
                                            color={color}
                                            onClick={() => {
                    ...
                    <div className={clsx(styles.formControl, styles.formMainDataItem, styles.formMainDataSmall)} >
                        ...
                        <FormControl className={clsx(styles.formMainDataSmallItem)}>
                            <TextField
                                ...
                                label="Exp"
                                variant="outlined"
                                defaultValue={DEFAULT_REMEMBER_VALUE ? DEFAULT_REMEMBER_VALUE?.AmountExpire : 3}
                                size="medium"
                                InputProps={{
                                    endAdornment: <InputAdornment position="end">
                                        m
                    ...
                    <FormControl className={clsx(styles.formControl, styles.formMainDataItem)}>
                        <TextField
                            ...
                            label="Expire"
                            variant="outlined"
                            defaultValue={DEFAULT_REMEMBER_VALUE ? DEFAULT_REMEMBER_VALUE?.Expire : 15}
                            size="medium"
                            InputProps={{
                                endAdornment: <InputAdornment position="end">
                                    m
                    ...
                    <FormControl className={clsx(styles.formControl, styles.formMainDataItem)}>
                        <TextField
                            ...
                            variant="outlined"
                            defaultValue={DEFAULT_REMEMBER_VALUE ? DEFAULT_REMEMBER_VALUE?.AmountIncreaseOC : 10}
                            size="medium"
                            InputProps={{
                                endAdornment: <InputAdornment position="end">
                                    %
            ...
            {spotMaxTradeAmountList.length === 1 && (
                <>
                    <b><b style={{
                        ...
                        color: " #db2f2f",
                        fontSize: "1rem",
                    }}>MAX</b>: {formatNumberString(spotMaxTradeAmountList[0].spotMaxTradeAmount)}$</b>
                    <b><b style={{
                        margin: "0 3px 0 10px",
                        color: " #db2f2f",
                        fontSize: "1rem",
                    }}>AVAI</b>: {formatNumberString(spotMaxTradeAmountList[0].availBal)}$</b>
                </>
            )}
            {spotMaxTradeAmountList.length > 1 && (
                <div style={{
                        ...
                        <Table className={styles.addMember}>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell style={{ fontWeight: "bold" }}>Avai </TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {
                                    showSpotBorrowList.map((data, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell>
                                                {formatNumberString(data.spotMaxTradeAmount)}
                                            </TableCell>
                                            <TableCell>
                                                {formatNumberString(data.availBal)}
                                            </TableCell>
                                        </TableRow>
                                    ))
                                }
