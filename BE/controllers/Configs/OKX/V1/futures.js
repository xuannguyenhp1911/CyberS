...
const dataCoinByBitController = {
    ...
    getFuturesBorrowCheck: async (req, res) => {
        ...
        try {
            const resultAll = await Promise.allSettled(botListData.map(async botData => {
                ...
                try {
                    ...
                    if (positionSide !== "Long") {
                        const getCurrentPriceRes = await client.getTicker({
                            instId: symbol
                        })
            ...
            res.customResponse(res.statusCode, "Get Futures Borrow Check Successful", handleResult);
        }
        catch (err) {
    ...
    getLever: async (req, res) => {
        ...
        try {
            ...
            res.customResponse(resData.statusCode, "Get Lever Successful", {
                "long": resData[0]?.lever,
                "short": resData[1]?.lever
            });
